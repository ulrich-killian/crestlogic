...
import { INITIAL_SHIPMENT_DATA, generateTrackingId, SAMPLE_INVOICES, getLatLng, getGeodesicDistance } from "./utils";
...
export default function App({
  ...
  const [domesticFormData, setDomesticFormData] = useState({
    ...
    paymentMethod: "Credit Card" as "Apple Pay" | "Credit Card" | "Cash App" | "PayPal" | "Other methods" | string,
    
    uspsPriority: false,
    uspsExpress: false,
    uspsPackageServices: false,
    
    fedexServiceTier: "" as "PriorityOvernight" | "StandardOvernight" | "FedEx2Day" | "ExpressSaver" | "Ground" | "",
  ...
  const aiRouteAnalysis = useMemo(() => {
    ...
    const defaultDest = { lat: 40.7128, lng: -74.0060 }; // NY
    ...
    if (originStr.toLowerCase().includes("canada") || destinationStr.toLowerCase().includes("canada")) {
      corridorName = "Trans-Border Northern Expansion Route";
    } else if (originStr.toLowerCase().includes("mexic") || destinationStr.toLowerCase().includes("mexic")) {
  ...
  }, [domesticFormData.senderAddress, domesticFormData.recipientAddress, domesticFormData.recipientCityStateZip]);
  const [domesticMessageDraft, setDomesticMessageDraft] = useState("");
  const [dispatchContactTarget, setDispatchContactTarget] = useState("");
  const [dispatchChannel, setDispatchChannel] = useState<"sms" | "email" | "whatsapp">("whatsapp");
  ...
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
      setTimeStr(now.toISOString().replace("T", " ").replace(/\..+/, "") + " UTC");
    };
    updateTime();
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            ...
            JSON.stringify(data.history) !== JSON.stringify(publicFoundShipment.history)
          ) {
  ...
  const handlePublicTrackSearch = async (e: React.FormEvent | null, specificCode?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Tracking waybill ID ${code} is not registered in our core database indexes.`);
      }
      const data = await response.json();
      setPublicFoundShipment(data);
    } catch (err: any) {
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    if (adminUsername.trim() === "admin" && adminPassword === "admin2026") {
      if (typeof document !== "undefined") {
        document.cookie = "admin_token=authenticated_operator; path=/; max-age=86400";
      }
      ...
      setAuthError("Authorized Personnel Only: Incorrect Operator credentials.");
    }
  };
  ...
  const handleSimulateInvoiceOcr = (fileName: string) => {
    ...
    const templateKey = Object.keys(SAMPLE_INVOICES).find(
      k => fileName.toLowerCase().includes(k.substring(0, 10)) || k.toLowerCase().includes(fileName.toLowerCase())
    );
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (templateKey && SAMPLE_INVOICES[templateKey]) {
          ...
          setParsingLogs("Industrial structural invoice parsed. Custom manifest generated.");
        }
  ...
  const handleUpdateCargoItemName = (id: string, name: string) => {
    const updated = adminFormData.items.map(item => item.id === id ? { ...item, name } : item);
    setAdminFormData({ ...adminFormData, items: updated });
  };

  const handleUpdateCargoItemQty = (id: string, qty: number) => {
    const updated = adminFormData.items.map(item => item.id === id ? { ...item, qty: Math.max(1, qty) } : item);
    setAdminFormData({ ...adminFormData, items: updated });
  };
  ...
  const handleTriggerAiOptimization = async () => {
    ...
    try {
      ...
      setAdminAnalysisError("Cognitive server delayed. Falling back to local heuristic modeling.");
    } finally {
  ...
  const handleRegisterShipmentTicket = async () => {
    ...
    try {
      ...
      console.error(e);
      setAdminAnalysisError("Registry error: Server database connection could not be committed.");
    }
  };
  ...
  const handleRegisterDomesticWaybill = async (e: React.FormEvent) => {
    ...
    if (!domesticFormData.recipientTo || !domesticFormData.recipientAddress || !domesticFormData.recipientCityStateZip) {
      setAdminAnalysisError("Cannot issue waybill ticket: Recipient's name, Address, and City/State/Zip are strictly required.");
      return;
    }
    if (!domesticFormData.sender) {
      setAdminAnalysisError("Cannot issue waybill ticket: Sender name is strictly required.");
      return;
    }
    ...
    const dynamicInsights = {
      suggestedCarrier: domesticFormData.fedexServiceTier ? `FedEx ${domesticFormData.fedexServiceTier}` : "U.S. Postal Service",
      predictedTransitDays: domesticFormData.fedexServiceTier === "PriorityOvernight" || domesticFormData.fedexServiceTier === "StandardOvernight" ? "1-2 Days" : "2-3 Days",
      riskAssessment: "LOW" as const,
      directives: [
        `Domestic package tracking routing successfully assigned standard protocol.`,
        `Deliver via chosen: ${domesticFormData.fedexServiceTier ? 'FedEx Delivery' : 'U.S. Postal Service'}.`
      ],
    ...
    const finalPayload = {
      formData: {
        ...
        destination: `${domesticFormData.recipientAddress}, ${domesticFormData.recipientCityStateZip}`,
        items: [{ id: `dom-item-1`, name: domesticFormData.fedexServiceTier ? `FedEx ${domesticFormData.fedexServiceTier} Shipment` : "USPS Parcel", qty: 1 }],
    ...
    try {
      ...
      const targetPhone = domesticFormData.recipientPhone || domesticFormData.senderPhone || "555-019-2834";
  ...
  const handleUpdateWaybillHubs = async () => {
    ...
    try {
      ...
      const progressPercent = totalDist > 0 ? (liveDistanceCovered / totalDist) * 100 : 0;
      ...
      const historyPoint = {
        status: nextStatus,
        location: progressPercent >= 100 ? liveDestinationInput : progressPercent <= 0 ? liveOriginInput : "In Route Overland Hub Segment",
        date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
        time: new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "numeric", hour12: true }),
        activity: `System-dispatched geolocation progress: Advanced driving telemetry to ${Math.round(progressPercent)}% milestone.`
      };
      ...
      const updatedShipment = {
        ...
        status: nextStatus,
        history: hasStatusChanged ? [...(focusedWaybill.history || []), historyPoint] : (focusedWaybill.history || [])
      };
      ...
      setDispatchStatusMsg("Route coordinate metrics updated live and synchronized with the public server!");
      setTimeout(() => setDispatchStatusMsg(null), 5000);

    } catch (err) {
      console.error(err);
      setDispatchStatusMsg("Error committing coordinate updates to server database.");
    } finally {
  ...
  const handleSendDynamicNotification = (isLinkOnly: boolean = false) => {
    ...
    setTimeout(() => {
      const modeText = dispatchChannel === "whatsapp" ? "WhatsApp" 
      ...
      setSimulatedNotifications(prev => [
        {
          ...
          title: isLinkOnly ? "Direct Shipment Link" : "Automated Waybill Dispatch",
          text: finalMessage,
      ...
      setDispatchStatusMsg(`Success! Automated message was queued and transmitted via ${modeText} to: ${dispatchContactTarget}`);
    }, 900);
  };
  ...
  const updateShipmentMilestoneOnServer = async (id: string, nextStatus: string) => {
    ...
    if (nextStatus === "DRY_BULK_SORTED") {
      desc = "Consolidation verification completed. Dynamic wrapping secure seals locked.";
      loc = "Crest Packaging Hub";
    } else if (nextStatus === "IN_OVERLAND_TRANSIT") {
      desc = "Secure container loaded onto heavy cargo fleet. Highway transit initiated.";
      loc = "Crest Logistics Gate 2";
    } else if (nextStatus === "GATEWAY_CUSTOMS_HOLD") {
      desc = "Cross-border verification protocol triggered. Custom clearing cargo holding.";
      loc = "Gateway Border Custody Checkpoint";
    } else if (nextStatus === "DELIVERED") {
      desc = "Consignee delivery handshake successfully executed. Waybill status closed.";
  ...
  const handleSendChatMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      let botText = "Thank you. Our dispatch centers are monitoring active Cameroon- Senegal channels. Could you supply your verified tracking barcode code?";
      const msg = userText.toLowerCase();
      if (msg.includes("track") || msg.includes("id") || msg.includes("cr-") || msg.includes("code")) {
        botText = "To search active shipping waybills, please enter your CR-XXXXXX-LT tracking number in the tracking widget or consult with our operator nodes.";
      } else if (msg.includes("admin") || msg.includes("portal") || msg.includes("login")) {
        botText = "Crest Admin console utilizes deep authentication metrics. Select the 'Authorized Portal Login' inside our page menu to access registered control charts.";
      } else if (msg.includes("about") || msg.includes("pricing") || msg.includes("contact")) {
        botText = "We establish dry-bulk multi-modal grids globally. Please fill out details in our 'Get a Free Quote' contact block below, and our dispatchers will return quotes instantly.";
      }
    ...
    <div className="min-h-screen bg-[#FAF5E9] dark:bg-[#070c0a] text-[#111E19] dark:text-stone-100 flex flex-col font-sans antialiased selection:bg-[#F7E4A1] selection:text-[#111E19]">
      ...
      <header className="w-full bg-white dark:bg-[#0c1411] border-b border-stone-200/60 dark:border-stone-800 shadow-xs sticky top-0 z-50 transition-colors">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex items-center gap-1.5">
            ...
            <button
              onClick={() => { setIsAdminMode(false); setActiveTab("track"); }}
              className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-all ${
          ...
          <div className="flex items-center gap-3">
            ...
            {!forcePublic && !forceAdminLogin && !forceAdminDashboard && (
              ...
              >
                ...
                <span className="hidden sm:inline">{isAdminMode ? "Exit Admin Control" : "Authorized Admin Login"}</span>
            ...
            <div className="flex-grow flex items-center justify-center p-6 bg-[#FAF5E9]">
              <div className="w-full max-w-md bg-white border border-stone-200/60 rounded-3xl p-8 shadow-lg flex flex-col gap-6">
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-sans font-black text-[#111E19] tracking-tight">Crest Operators Portal</h2>
                  <p className="text-xs text-stone-500 font-sans mt-2">
                    Secured connection portal for terminal admins and dispatch agents.
                  </p>
                </div>
                ...
                <form onSubmit={handleAdminLogin} className="flex flex-col gap-4 font-sans text-xs">
                  <div>
                  ...
                  <div className="bg-amber-50/70 border border-amber-200 p-3 rounded-xl flex gap-2 items-start mt-1">
                    ...
                    <p className="text-[10px] text-stone-600 leading-normal">
                      <strong>Demo Access Credentials:</strong> Use username <span className="font-mono font-bold bg-white px-1 py-0.5 border border-amber-250 rounded">admin</span> and password <span className="font-mono font-bold bg-white px-1 py-0.5 border border-amber-250 rounded">admin2026</span> to bypass.
                    </p>
                  </div>
            ...
            <div className="flex-grow flex flex-col lg:flex-row bg-[#FAF5E9]">
              ...
              <aside className="w-full lg:w-64 bg-white border-b lg:border-r border-stone-200/60 p-5 flex flex-col gap-6 shrink-0">
                <div className="pb-4 border-b border-stone-100 flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-sans font-black text-sm text-[#111E19] tracking-tight">Crest Tower Node</h3>
                    <p className="text-[10px] text-stone-400 font-mono">STATUS: OPERATIONAL</p>
                  </div>
                </div>
                ...
                <nav className="flex flex-row lg:flex-col gap-1.5 overflow-x-auto lg:overflow-visible font-sans">
                  ...
                  <button
                    onClick={() => { setAdminSection("registry"); fetchShipmentsDb(); }}
                    className={`w-full text-left px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-2.5 shrink-0 transition-all ${
                  ...
                  >
                    <Sliders className="w-4 h-4" />
                    <span>Direct Archive Database ({serverShipments.length})</span>
                  </button>
                  <button
                ...
                <div className="hidden lg:flex flex-col gap-3 mt-auto p-4 bg-stone-50 rounded-2xl border border-stone-250">
                  ...
                  <p className="text-[11px] text-stone-500 font-sans leading-relaxed">
                    Connected to regional CRM central database index. Your registered actions log under: <span className="font-mono text-stone-700 font-bold">Terminal-883A</span>.
                  </p>
                  <button
                    onClick={() => {
                      if (typeof document !== "undefined") {
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white border border-stone-200/60 p-5 rounded-2xl shadow-2xs">
                  <div>
                    ...
                    <p className="text-xs text-stone-500 font-sans mt-0.5">
                      Generate, analyze, and dispatch live cargo containers in Cameroon and international transits.
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-8 w-full">
                        ...
                        <div className="bg-white border-2 border-[#A35638]/30 rounded-2xl p-6 shadow-md flex flex-col md:flex-row gap-6 animate-fadeIn">
                          ...
                          <div className="md:w-1/2 flex flex-col gap-4">
                            <div className="flex items-center gap-3 bg-emerald-50 text-emerald-800 border border-emerald-200/50 px-4 py-3 rounded-xl">
                              ...
                              <div>
                                <h4 className="font-sans font-black text-sm uppercase">Waybill Issued Successfully!</h4>
                                <p className="text-[11px] opacity-90 mt-0.5">Domestic ticket has been written to the Crest Operations Live Registry.</p>
                              </div>
                            </div>

                            <div className="border border-stone-200 rounded-xl p-4 bg-stone-50/50 flex flex-col gap-2">
                              <span className="text-[10px] font-mono uppercase tracking-wider text-stone-400 font-extrabold">UNIQUE TRACKING ID</span>
                              <div className="flex items-center justify-between">
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    navigator.clipboard.writeText(lastCreatedDomesticWaybill.orderId);
                                    setDispatchStatusMsg("Tracking ID copied to clipboard!");
                                  }}
                            ...
                            <div className="border border-stone-150 rounded-xl p-4 flex flex-col gap-1.5 bg-white text-xs text-stone-700">
                              <h5 className="font-sans font-black text-[#111E19] uppercase text-[10px] tracking-wider mb-1">RECIPIENT SUMMARY</h5>
                              <p><strong>To:</strong> {lastCreatedDomesticWaybill.customerName}</p>
                              <p><strong>Destination:</strong> {lastCreatedDomesticWaybill.destination}</p>
                              <p><strong>Service Type:</strong> {domesticFormData.fedexServiceTier ? `FedEx ${domesticFormData.fedexServiceTier}` : `USPS ${domesticFormData.uspsPriority ? 'Priority' : domesticFormData.uspsExpress ? 'Express' : 'Package Services'}`}</p>
                            </div>
                            ...
                            <div className="border border-stone-200 rounded-2xl p-4 bg-stone-50 flex flex-col gap-3">
                              <div className="flex items-center gap-1.5 border-b border-stone-200 pb-2">
                                ...
                                <h5 className="font-sans font-black text-[#111E19] uppercase text-[10px] tracking-wider">LIVE ROUTE INTERACTION</h5>
                              </div>
                              ...
                              <div className="flex flex-col gap-2">
                                <div>
                                  <label className="block text-[8px] font-mono uppercase text-stone-400 font-extrabold mb-1 flex items-center gap-1">
                                    ...
                                    Sender Location Address (Origin started address) [Locked]
                                  </label>
                                  <input 
                                    type="text" 
                                ...
                                <div>
                                  ...
                                  <input 
                                    ...
                                    onChange={(e) => setLiveCheckpointInput(e.target.value)}
                                ...
                                <div>
                                  <label className="block text-[8px] font-mono uppercase text-stone-400 font-extrabold mb-1 flex items-center gap-1">
                                    <span className="w-1 h-1 rounded-full bg-stone-400" />
                                    Receiver Location Address (Destination delivery point) [Locked]
                                  </label>
                                  <input 
                                    type="text" 
                                    className="w-full text-xs font-mono rounded-xl border border-stone-200 px-3 py-1.5 text-stone-400 bg-stone-50/50 cursor-not-allowed"
                          ...
                          <div className="md:w-1/2 border-l border-stone-100 md:pl-6 flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-2">
                              ...
                              <input
                                ...
                                onChange={(e) => setDispatchContactTarget(e.target.value)}
                              />
                            </div>
                              ...
                              <div className="grid grid-cols-3 gap-1.5 bg-stone-100 p-1 rounded-xl">
                                <button
                                  ...
                                  className={`py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all flex items-center justify-center gap-1 ${
                                    dispatchChannel === "whatsapp" ? "bg-[#111E19] text-[#F7E4A1]" : "text-stone-500 hover:text-stone-800"
                                  }`}
                                >
                                ...
                                <button
                                  ...
                                  className={`py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all flex items-center justify-center gap-1 ${
                                    dispatchChannel === "sms" ? "bg-[#111E19] text-[#F7E4A1]" : "text-stone-500 hover:text-stone-800"
                                  }`}
                                >
                                ...
                                <button
                                  ...
                                  className={`py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all flex items-center justify-center gap-1 ${
                                    dispatchChannel === "email" ? "bg-[#111E19] text-[#F7E4A1]" : "text-stone-500 hover:text-stone-800"
                                  }`}
                                >
                            ...
                            <div className="flex flex-col gap-1.5">
                              ...
                              <textarea
                                ...
                                onChange={(e) => setDomesticMessageDraft(e.target.value)}
                              />
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-1">
                              <button
                                type="button"
                                onClick={() => handleSendDynamicNotification(false)}
                              ...
                              <button
                                type="button"
                                onClick={() => handleSendDynamicNotification(true)}
                            ...
                            <div className="flex flex-col gap-4 border-r-0 md:border-r border-stone-100 md:pr-6">
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                <div className="col-span-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, shipDate: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, sender: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, senderAddress: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-1">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, senderPhone: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-1">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, cmNumber: e.target.value })}
                                  />
                                </div>
                              </div>
                              ...
                              <div className="bg-stone-50/70 border border-stone-200/50 p-3 rounded-2xl flex flex-col gap-2">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                  <div>
                                    <label className="block text-[9px] font-sans uppercase text-stone-500 mb-0.5">Fund</label>
                                    <input
                                      type="text"
                                      placeholder="Fund"
                                      ...
                                      onChange={(e) => setDomesticFormData({ ...domesticFormData, fund: e.target.value })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-[9px] font-sans uppercase text-stone-500 mb-0.5">Org</label>
                                    <input
                                      type="text"
                                      placeholder="Org"
                                      className="w-full text-xs text-center font-sans rounded-lg border border-stone-300 px-2.5 py-1.5 focus:outline-none bg-white font-mono"
                                      value={domesticFormData.org}
                                      onChange={(e) => setDomesticFormData({ ...domesticFormData, org: e.target.value })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-[9px] font-sans uppercase text-stone-500 mb-0.5">Acct</label>
                                    <input
                                      type="text"
                                      placeholder="Acct"
                                      className="w-full text-xs text-center font-sans rounded-lg border border-stone-300 px-2.5 py-1.5 focus:outline-none bg-white font-mono"
                                      value={domesticFormData.acct}
                                      onChange={(e) => setDomesticFormData({ ...domesticFormData, acct: e.target.value })}
                                    />
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="flex flex-col gap-4">
                              <div className="flex items-center justify-between border-b border-stone-100 pb-1.5">
                                ...
                                <div className="flex bg-stone-100 border border-stone-200 p-0.5 rounded-lg text-[9px] font-sans font-bold">
                                  <button
                                    type="button"
                                    onClick={() => setDomesticFormData({ ...domesticFormData, recipientAddressType: "Business" })}
                                    className={`px-2 py-0.5 rounded transition-all ${domesticFormData.recipientAddressType === "Business" ? "bg-[#111E19] text-[#F7E4A1] shadow-2xs" : "text-stone-500 hover:text-stone-850"}`}
                                  >
                                    Business
                                  </button>
                                  <button
                                    type="button"
                                    onClick={() => setDomesticFormData({ ...domesticFormData, recipientAddressType: "Residential" })}
                                    className={`px-2 py-0.5 rounded transition-all ${domesticFormData.recipientAddressType === "Residential" ? "bg-[#111E19] text-[#F7E4A1] shadow-2xs" : "text-stone-500 hover:text-stone-850"}`}
                                  >
                                    Residential
                                  </button>
                                </div>
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                <div className="col-span-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, recipientTo: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, recipientCompany: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, recipientAddress: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-1">
                                  ...
                                  <input
                                    ...
                                    className="w-full text-xs font-sans rounded-xl border border-stone-300 px-3.5 py-2.5 focus:outline-none focus:border-[#A35638] text-stone-800 bg-white"
                                    value={domesticFormData.recipientCityStateZip}
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, recipientCityStateZip: e.target.value })}
                                  />
                                </div>
                                ...
                                <div className="col-span-1">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, recipientPhone: e.target.value })}
                                  />
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="bg-[#111E19] border border-stone-800 text-stone-100 rounded-3xl p-5 md:p-6 shadow-md flex flex-col md:flex-row justify-between gap-6 items-stretch animate-fadeIn">
                              <div className="flex-1 flex flex-col justify-between">
                                ...
                                <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mt-4 text-xs font-mono">
                                  <div>
                                    <span className="text-stone-400 block text-[9px] uppercase font-bold tracking-wider">Total Geonotation:</span>
                                    <span className="font-semibold text-emerald-400">{aiRouteAnalysis.distanceKm} km</span> ({aiRouteAnalysis.distanceMiles} miles)
                                  </div>
                                  <div>
                                    <span className="text-stone-400 block text-[9px] uppercase font-bold tracking-wider">Est. Traveling Days:</span>
                                    <span className="font-extrabold text-[#F7E4A1] text-sm leading-none">{aiRouteAnalysis.travellingDays} Days</span>
                                  </div>
                                  <div className="col-span-2 lg:col-span-1">
                                    <span className="text-stone-400 block text-[9px] uppercase font-bold tracking-wider">Continuous Hours:</span>
                                    <span className="font-bold text-stone-200">{aiRouteAnalysis.travellingHours} hrs driving</span>
                                  </div>
                                </div>
                              </div>
                              <div className="w-full md:w-[220px] bg-emerald-950/40 border border-emerald-900/30 rounded-2xl p-4 flex flex-col justify-center text-center">
                                ...
                                <p className="text-[10px] text-stone-300 font-medium leading-relaxed">
                                  System-assigned overland corridor maps real-time driving. At standard 80 km/h logistics speed, route yields transit period of <strong className="text-white">{aiRouteAnalysis.travellingDays} days</strong> no matter what.
                                </p>
                              </div>
                            </div>
                          )}
                              ...
                              <div className="flex flex-col gap-2.5 text-xs text-stone-700 font-sans">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    ...
                                    name="paymentType"
                                    className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                    checked={domesticFormData.paymentBillShipper}
                                    onChange={() => setDomesticFormData({
                                      ...domesticFormData,
                                      paymentPersonal: false,
                                      paymentBusiness: false,
                                ...
                                <div className="flex flex-col gap-1.5">
                                  <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                    <input
                                      ...
                                      name="paymentType"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.paymentBill3rdParty}
                                      onChange={() => setDomesticFormData({
                                        ...domesticFormData,
                                        paymentPersonal: false,
                                        paymentBusiness: false,
                                  ...
                                  {domesticFormData.paymentBill3rdParty && (
                                    <input
                                      ...
                                      value={domesticFormData.paymentThirdPartyAcct}
                                      onChange={(e) => setDomesticFormData({ ...domesticFormData, paymentThirdPartyAcct: e.target.value })}
                                    />
                                  )}
                                </div>
                                ...
                                <div className="flex flex-col gap-1.5">
                                  <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                    <input
                                      ...
                                      name="paymentType"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.paymentBillRecipient}
                                      onChange={() => setDomesticFormData({
                                        ...domesticFormData,
                                        paymentPersonal: false,
                                        paymentBusiness: false,
                                  ...
                                  {domesticFormData.paymentBillRecipient && (
                                    <input
                                      ...
                                      className="w-full text-[11px] font-mono rounded-lg border border-stone-300 px-2.5 py-1.5 focus:outline-none focus:border-[#A35638] bg-white text-stone-800 ml-5 w-[calc(100%-24px)] animate-fadeIn"
                                      value={domesticFormData.paymentRecipientAcct}
                                      onChange={(e) => setDomesticFormData({ ...domesticFormData, paymentRecipientAcct: e.target.value })}
                                    />
                                  )}
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col gap-2.5 text-xs text-stone-700 font-sans">
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, uspsPriority: e.target.checked })}
                                  />
                                  <div className="flex flex-col">
                                    <span>Priority Mail (2-3 Days Estimated Delivery)</span>
                                    <span className="text-[9px] text-stone-400">Domestic flat-rate packaging files</span>
                                  </div>
                                </label>
                                ...
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, uspsExpress: e.target.checked })}
                                  />
                                  <div className="flex flex-col">
                                    <span>Express Mail (1-2 Days Guaranteed Overnight)</span>
                                    <span className="text-[9px] text-stone-400">Signature required flat dispatch option available</span>
                                  </div>
                                </label>
                                ...
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    type="checkbox"
                                    className="rounded text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                    checked={domesticFormData.uspsPackageServices}
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, uspsPackageServices: e.target.checked })}
                                  />
                                  <div className="flex flex-col">
                                    <span>Package Services (All other structural courier classes)</span>
                                    <span className="text-[9px] text-stone-400">Library and Media Cargo classifications included</span>
                                  </div>
                                </label>
                              </div>
                            </div>
                                ...
                                <div className="pb-1 border-b border-stone-150">
                                  <label className="flex items-center gap-1.5 cursor-pointer font-bold select-none text-[#111E19]">
                                    <input
                                      ...
                                      name="fedexTier"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.fedexServiceTier === "PriorityOvernight"}
                                      onChange={() => setDomesticFormData({ ...domesticFormData, fedexServiceTier: "PriorityOvernight" })}
                                    />
                                    <span>Priority Overnight</span>
                                  </label>
                                  <span className="text-[9px] block text-stone-400 font-sans mt-0.5 leading-none">Delivered next business a.m.</span>
                                </div>
                                
                                {domesticFormData.fedexServiceTier === "PriorityOvernight" && (
                                  <div className="flex flex-col gap-1.5 text-[10px] pl-0.5 font-bold animate-fadeIn">
                                    <span className="text-[8px] uppercase tracking-wider text-[#A35638] font-bold">Packaging type</span>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_po" checked={domesticFormData.fedexPackaging === "letter"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "letter"})} /> Letter</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_po" checked={domesticFormData.fedexPackaging === "pak"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "pak"})} /> FedEx Pak</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_po" checked={domesticFormData.fedexPackaging === "box"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "box"})} /> FedEx Box</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_po" checked={domesticFormData.fedexPackaging === "other"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "other"})} /> Other Packaging</label>
                                  </div>
                                )}
                              </div>
                                ...
                                <div className="pb-1 border-b border-stone-150">
                                  <label className="flex items-center gap-1.5 cursor-pointer font-bold select-none text-[#111E19]">
                                    <input
                                      ...
                                      name="fedexTier"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.fedexServiceTier === "StandardOvernight"}
                                      onChange={() => setDomesticFormData({ ...domesticFormData, fedexServiceTier: "StandardOvernight" })}
                                    />
                                    <span>Standard Overnight</span>
                                  </label>
                                  <span className="text-[9px] block text-stone-400 font-sans mt-0.5 leading-none">Delivered next business p.m.</span>
                                </div>
                                
                                {domesticFormData.fedexServiceTier === "StandardOvernight" && (
                                  <div className="flex flex-col gap-1.5 text-[10px] pl-0.5 font-bold animate-fadeIn">
                                    <span className="text-[8px] uppercase tracking-wider text-[#A35638] font-bold">Packaging type</span>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_so" checked={domesticFormData.fedexPackaging === "letter"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "letter"})} /> Letter</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_so" checked={domesticFormData.fedexPackaging === "pak"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "pak"})} /> FedEx Pak</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_so" checked={domesticFormData.fedexPackaging === "box"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "box"})} /> FedEx Box</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_so" checked={domesticFormData.fedexPackaging === "other"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "other"})} /> Other Packaging</label>
                                  </div>
                                )}
                              </div>
                                ...
                                <div className="pb-1 border-b border-stone-150">
                                  <label className="flex items-center gap-1.5 cursor-pointer font-bold select-none text-[#111E19]">
                                    <input
                                      ...
                                      name="fedexTier"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.fedexServiceTier === "FedEx2Day"}
                                      onChange={() => setDomesticFormData({ ...domesticFormData, fedexServiceTier: "FedEx2Day" })}
                                    />
                                    <span>FedEx 2-Day</span>
                                  </label>
                                  <span className="text-[9px] block text-stone-400 font-sans mt-0.5 leading-none">Delivered inside 2 business days</span>
                                </div>
                                
                                {domesticFormData.fedexServiceTier === "FedEx2Day" && (
                                  <div className="flex flex-col gap-1.5 text-[10px] pl-0.5 font-bold animate-fadeIn">
                                    <span className="text-[8px] uppercase tracking-wider text-[#A35638] font-bold">Packaging type</span>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_2d" checked={domesticFormData.fedexPackaging === "letter"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "letter"})} /> Letter</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_2d" checked={domesticFormData.fedexPackaging === "pak"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "pak"})} /> FedEx Pak</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_2d" checked={domesticFormData.fedexPackaging === "box"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "box"})} /> FedEx Box</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_2d" checked={domesticFormData.fedexPackaging === "other"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "other"})} /> Other Packaging</label>
                                  </div>
                                )}
                              </div>
                                ...
                                <div className="pb-1 border-b border-stone-150">
                                  <label className="flex items-center gap-1.5 cursor-pointer font-bold select-none text-[#111E19]">
                                    <input
                                      ...
                                      name="fedexTier"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.fedexServiceTier === "ExpressSaver"}
                                      onChange={() => setDomesticFormData({ ...domesticFormData, fedexServiceTier: "ExpressSaver" })}
                                    />
                                    <span>Express Saver</span>
                                  </label>
                                  <span className="text-[9px] block text-stone-400 font-sans mt-0.5 leading-none">Delivered inside 3 business days</span>
                                </div>
                                
                                {domesticFormData.fedexServiceTier === "ExpressSaver" && (
                                  <div className="flex flex-col gap-1.5 text-[10px] pl-0.5 font-bold animate-fadeIn">
                                    <span className="text-[8px] uppercase tracking-wider text-[#A35638] font-bold">Packaging type</span>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_es" checked={domesticFormData.fedexPackaging === "letter"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "letter"})} /> Letter</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_es" checked={domesticFormData.fedexPackaging === "pak"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "pak"})} /> FedEx Pak</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_es" checked={domesticFormData.fedexPackaging === "box"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "box"})} /> FedEx Box</label>
                                    <label className="flex items-center gap-1.5 cursor-pointer"><input type="radio" name="pkg_es" checked={domesticFormData.fedexPackaging === "other"} onChange={() => setDomesticFormData({...domesticFormData, fedexPackaging: "other"})} /> Other Packaging</label>
                                  </div>
                                )}
                              </div>
                                ...
                                <div className="pb-1 border-b border-stone-150">
                                  <label className="flex items-center gap-1.5 cursor-pointer font-bold select-none text-[#111E19]">
                                    <input
                                      ...
                                      name="fedexTier"
                                      className="text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                      checked={domesticFormData.fedexServiceTier === "Ground"}
                                      onChange={() => setDomesticFormData({ ...domesticFormData, fedexServiceTier: "Ground" })}
                                    />
                                    <span>FedEx Ground</span>
                                  </label>
                                  <span className="text-[9px] block text-stone-400 font-sans mt-0.5 leading-none">Delivered within 1 to 6 bus days</span>
                                </div>
                                ...
                                {domesticFormData.fedexServiceTier === "Ground" && (
                                  <div className="flex flex-col gap-1.5 text-[10px] pl-0.5 font-bold animate-fadeIn">
                                    ...
                                    <label className="flex items-center gap-1.5 cursor-none font-medium bg-[#F7E4A1]/30 p-1 rounded border border-[#F7E4A1] text-[9px] text-[#A35638] leading-tight select-none">
                                      Standard Ground Cargo Packaging Box Assigned
                                    </label>
                                  </div>
                                )}
                              </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-stone-100 pt-5">
                              ...
                              <div className="flex flex-col gap-3 text-xs text-stone-700 font-sans pt-1">
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    type="checkbox"
                                    className="rounded text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                    checked={domesticFormData.surchargeDirectSignature}
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, surchargeDirectSignature: e.target.checked })}
                                  />
                                  <span>Direct Recipient Handoff Signature Required</span>
                                </label>
                                ...
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    type="checkbox"
                                    className="rounded text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                    checked={domesticFormData.surchargeSaturdayDelivery}
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, surchargeSaturdayDelivery: e.target.checked })}
                                  />
                                  <span>Guaranteed Saturday Weekend Delivery</span>
                                </label>
                                ...
                                <label className="flex items-center gap-2 cursor-pointer font-medium selection:bg-none">
                                  <input
                                    type="checkbox"
                                    className="rounded text-[#A35638] focus:ring-[#A35638] border-stone-300"
                                    checked={domesticFormData.surchargeHolidayDelivery}
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, surchargeHolidayDelivery: e.target.checked })}
                                  />
                                  <span>Priority Holiday Cargo Window Option</span>
                                </label>
                              </div>
                            </div>
                            ...
                            <div className="bg-[#FAF5E9]/50 border border-[#FAF5E9] p-4 rounded-2xl flex flex-col gap-3">
                              ...
                              <p className="text-[10px] font-sans text-stone-500 leading-normal">
                                First $100 value of commercial cargo package listed is protected automatically by general courier mandates.
                              </p>
                              ...
                              <div className="flex flex-col gap-3 font-sans text-xs pt-1">
                                <label className="flex items-center gap-2 cursor-pointer font-bold select-none text-[#111E19]">
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, insuranceRequired: e.target.checked })}
                                  />
                                  <span>Additional Cargo Insurance Declared</span>
                                </label>
                                ...
                                {domesticFormData.insuranceRequired && (
                                  <div className="animate-fadeIn">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => setDomesticFormData({ ...domesticFormData, declaredValue: e.target.value })}
                                    />
                                  </div>
                                )}
                              </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <div className="flex rounded-xl overflow-hidden border border-stone-300">
                                  <input
                                    ...
                                    onChange={(e) => setDomesticFormData({ ...domesticFormData, packageWeight: e.target.value })}
                                  />
                                  <select
                                    ...
                                    onChange={(e: any) => setDomesticFormData({ ...domesticFormData, weightUnit: e.target.value })}
                                  >
                              ...
                              <div>
                                ...
                                <select
                                  ...
                                  onChange={(e) => setDomesticFormData({ ...domesticFormData, paymentMethod: e.target.value })}
                                >
                          ...
                          <div className="border-t border-stone-150 pt-5 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans text-stone-600">
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) => setDomesticFormData({ ...domesticFormData, processedBy: e.target.value })}
                              />
                            </div>
                            <div>
                              <label className="block text-[9px] uppercase tracking-wider text-stone-400 font-semibold mb-1">PROCESSING TIMESTAMP RECORD</label>
                              <input
                                type="text"
                                readOnly
                   ...
                   <div className="flex flex-col gap-3 pt-3 border-t border-stone-100">
                            ...
                            >
                              ...
                              <span>{isAdminAnalyzing ? "AI REGISTRY COMMITMENT IN PROGRESS..." : "Commit Manifest & Register Package"}</span>
                            </button>
                    ...
                    <div className="flex justify-between items-center pb-3 border-b border-stone-100 flex-col lg:flex-row gap-3">
                      <div>
                        ...
                        <p className="text-xs text-stone-400 mt-0.5 font-sans">
                          Active container manifests stored on Crest Logistics in-memory servers.
                        </p>
                      </div>
                      ...
                      <table className="w-full text-left font-sans text-xs border-collapse">
                        <thead>
                          <tr className="bg-stone-50 text-stone-500 font-mono text-[10px] uppercase font-bold tracking-wider border-b border-stone-200/80">
                            ...
                            <th className="py-3 px-4">Operational Status Milestone</th>
                        ...
                        <tbody className="divide-y divide-stone-100">
                          {serverShipments
                            .filter(s => {
                              ...
                              return (s.orderId || "").toLowerCase().includes(q) || (s.customerName || "").toLowerCase().includes(q) || (s.destination || "").toLowerCase().includes(q);
                            })
                            .sort((a, b) => {
                              if (sortField.startsWith("date")) {
                                const dateA = a.shipDate || (a.history && a.history[0]?.date) || "";
                                const dateB = b.shipDate || (b.history && b.history[0]?.date) || "";
                            ...
                            .map(shipment => (
                              <tr key={shipment.orderId} className="hover:bg-stone-50/50 text-stone-700 transition-colors">
                                ...
                                <td className="py-3 px-4">
                                  ...
                                  <select
                                    className={`text-[11px] font-sans font-extrabold uppercase rounded-lg border border-stone-200 px-2 py-1 focus:outline-none focus:border-[#A35638] ${
                                      ...
                                      shipment.status === "DRY_BULK_SORTED" ? "bg-amber-50 text-amber-700" :
                                      shipment.status === "IN_TRANSIT" || shipment.status === "IN_OVERLAND_TRANSIT" ? "bg-sky-50 text-sky-800" :
                                      shipment.status === "GATEWAY_CUSTOMS_HOLD" ? "bg-rose-50 text-rose-800" :
                                      "bg-emerald-50 text-emerald-800"
                                    }`}
                                    value={shipment.status}
                                    onChange={(e) => updateShipmentMilestoneOnServer(shipment.orderId, e.target.value)}
                                  >
                                    <option value="MANIFEST_CREATED">Manifest Created</option>
                                    <option value="DRY_BULK_SORTED">Consolidated & Sorted</option>
                                    <option value="IN_OVERLAND_TRANSIT">In Overland Transit</option>
                                    <option value="GATEWAY_CUSTOMS_HOLD">Customs Border Hold</option>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                    ...
                    <div className="lg:col-span-6 bg-white border border-stone-200/60 rounded-3xl p-5 md:p-7 shadow-xs flex flex-col gap-6">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-stone-500 font-sans mt-0.5">
                          Simulate pushing instant text notifications, bills of lading, and tracking codes directly to the consignee's phone via terminal websockets.
                        </p>
                      </div>
                      ...
                      {activeWaybill ? (
                        <div className="bg-[#FAF5E9] border border-stone-250 p-4 rounded-2xl flex flex-col gap-3 font-sans text-xs">
                          <div className="flex justify-between items-center pb-2 border-b border-stone-250/50">
                            <span className="font-mono text-[10px] text-stone-500 font-bold uppercase tracking-wider">Active Waybill focus</span>
                            <span className="font-mono text-xs font-black text-[#A35638]">{activeWaybill.orderId}</span>
                          </div>
                          
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <p className="text-[10px] text-stone-400 font-bold leading-tight uppercase">Recipient</p>
                              <p className="font-extrabold text-[#111E19] mt-0.5">{activeWaybill.customerName}</p>
                            </div>
                            <div>
                              <p className="text-[10px] text-stone-400 font-bold leading-tight uppercase">Cargo Weight</p>
                              <p className="font-extrabold text-[#111E19] mt-0.5 font-mono">{activeWaybill.weight} kg</p>
                            </div>
                          </div>
                          ...
                          <div className="border border-stone-250/70 bg-white rounded-xl p-3 flex flex-col gap-2.5 shadow-3xs">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[9px] font-sans font-extrabold text-[#111E19] uppercase tracking-wider">Live Route Alteration</span>
                            </div>
                              ...
                              <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 flex flex-col gap-3">
                                <div className="flex justify-between items-center text-[10px] font-sans font-black uppercase text-[#111E19]">
                                  ...
                                  <span className="font-mono text-emerald-600">
                                    {Math.round(adminTravelDistance > 0 ? (liveDistanceCovered / adminTravelDistance) * 100 : 0)}% Covered
                                  </span>
                                </div>
                                ...
                                <div className="w-full bg-stone-200 h-2 rounded-full overflow-hidden">
                                  <div 
                                    ...
                                    style={{ width: `${adminTravelDistance > 0 ? Math.min(100, (liveDistanceCovered / adminTravelDistance) * 100) : 0}%` }}
                                  />
                                </div>

                                <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-stone-600">
                                  <div>
                                    <span className="text-stone-400">Hours Driven:</span>
                                    <span className="block text-xs font-black text-[#111E19]">{liveHoursDriven} hrs</span>
                                  </div>
                                  <div>
                                    <span className="text-stone-400">Distance Covered:</span>
                                    <span className="block text-xs font-black text-[#111E19]">{Math.round(liveDistanceCovered)} / {adminTravelDistance} km</span>
                                  </div>
                                </div>
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        const nextDst = Math.min(adminTravelDistance, nextHrs * 80);
                          ...
                          <div className="border-t border-stone-250/50 pt-3 flex flex-col gap-2.5">
                            <p className="text-[10px] font-mono uppercase tracking-wider text-[#A35638] font-bold">Trigger Recipient Alerts:</p>
                            
                            <button
                              disabled={isAlertSending}
                              onClick={() => simulateRecipientDispatch("whatsapp")}
                        ...
                        <div className="bg-stone-50 border border-stone-200 rounded-2xl p-6 text-center text-xs text-stone-400">
                          ...
                          <p className="font-medium">No waybill is currently focused for dispatch simulators.</p>
                          <p className="text-[10px] text-stone-400 mt-1 max-w-sm mx-auto">
                            Go back to the <strong className="cursor-pointer text-[#A35638]" onClick={() => setAdminSection("register")}>&ldquo;Create & Optimize Waybill&rdquo;</strong> tab, fill out details, and issue a package ticket to trigger.
                          </p>
                        </div>
                      )}
                      ...
                      {isAlertSending && (
                        <div className="flex justify-center items-center gap-2 py-4">
                          ...
                          <span className="text-xs font-mono text-stone-500">Broadcasting notification wave...</span>
                        </div>
                      )}
                            ...
                            <AnimatePresence>
                              {simulatedNotifications.length > 0 ? (
                                simulatedNotifications.map((notif, index) => (
                                  ...
                                  >
                                    <div className="flex justify-between items-center bg-white/5 px-2.5 py-1 rounded-md text-[9px] font-mono tracking-wider">
                                      <span className="text-[#F7E4A1] font-bold">{notif.title}</span>
                                      <span className="text-stone-400">{notif.time}</span>
                                    </div>
                                    <div>
                                ...
                                <div className="text-center py-20 text-stone-500 text-xs flex flex-col items-center justify-center gap-3">
                                  ...
                                  <p className="max-w-xs leading-relaxed">No wireless signals received.</p>
                                  <p className="text-[9px] leading-normal opacity-70">
                                    Trigger WhatsApp, SMS, or Bill of Lading alerts in the dispatch panel to simulate push notification arrivals.
                                  </p>
                                </div>
                              )}
                            </AnimatePresence>
            ...
            <div className="flex flex-col">
              ...
              >
                ...
                <div className="relative max-w-5xl mx-auto w-full text-center flex flex-col items-center z-10 text-white px-2">
                  ...
                  >
                    ...
                    <span className="text-[#F7E4A1]">Solutions in Logistics</span>
                  </motion.h1>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full max-w-4xl mt-14 pt-8 border-t border-white/15 text-center">
                    <div>
                      ...
                      <p className="text-stone-400 text-[10px] sm:text-xs font-sans uppercase tracking-wider mt-1">On-Time Port Release</p>
                    </div>
                    <div>
                      <p className="text-[#F7E4A1] text-xl sm:text-2xl font-mono font-black">250k+</p>
                      <p className="text-stone-400 text-[10px] sm:text-xs font-sans uppercase tracking-wider mt-1">Containers Dispatched</p>
                    </div>
                    <div>
                      <p className="text-[#F7E4A1] text-xl sm:text-2xl font-mono font-black">&lt; 700ms</p>
                      <p className="text-stone-400 text-[10px] sm:text-xs font-sans uppercase tracking-wider mt-1">Tracking Sync Speed</p>
                    </div>
                    <div>
                      <p className="text-[#F7E4A1] text-xl sm:text-2xl font-mono font-black">CAMEROON</p>
                      <p className="text-stone-400 text-[10px] sm:text-xs font-sans uppercase tracking-wider mt-1">Regional Gateway Hub</p>
                    </div>
                  </div>
              ...
              <div className="bg-[#FAF5E9]/50 py-20 px-6 border-t border-stone-200/50">
                <div className="max-w-6xl mx-auto flex flex-col gap-10">
                  <div className="text-center">
                    <span className="text-[10px] font-mono tracking-widest text-[#A35638] font-black uppercase">Fleet Operations Showcase</span>
                    <h3 className="font-sans font-black text-[#111E19] text-2xl md:text-3xl mt-1 leading-tight uppercase">High-Fidelity Shipment Workflows</h3>
                    <p className="text-xs text-stone-500 mt-2 max-w-xl mx-auto leading-normal">
                      Explore actual operational environments representing real-time dry-bulk cargo consignments and carrier transit grids.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white border border-stone-200/60 rounded-3xl overflow-hidden shadow-xs hover:shadow-md transition-shadow group">
                      ...
                      <div className="p-5 flex flex-col gap-1.5">
                        <h4 className="font-sans font-black text-sm text-[#111E19] uppercase">Deepsea Container Terminal</h4>
                        <p className="text-[11px] text-stone-500 font-sans leading-relaxed">
                          Precision logistics crane systems coordinates heavy bulk shipments onto trans-ocean liner corridors safely.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-stone-200/60 rounded-3xl overflow-hidden shadow-xs hover:shadow-md transition-shadow group">
                      ...
                      <div className="p-5 flex flex-col gap-1.5">
                        <h4 className="font-sans font-black text-sm text-[#111E19] uppercase">Express Trucking Fleet</h4>
                        <p className="text-[11px] text-stone-500 font-sans leading-relaxed">
                          Multi-axle dry cargo trucks crossing bilateral border ingress grids to ensure swift, weather-independent transit.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border border-stone-200/60 rounded-3xl overflow-hidden shadow-xs hover:shadow-md transition-shadow group">
                      ...
                      <div className="p-5 flex flex-col gap-1.5">
                        <h4 className="font-sans font-black text-sm text-[#111E19] uppercase">Smart Logistics Center</h4>
                        <p className="text-[11px] text-stone-500 font-sans leading-relaxed">
                          Thermal monitored warehouse racks containing fragile and automated packing crates under strict security clearance.
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="py-16 md:py-20 px-6 border-t border-stone-200/50 dark:border-stone-850/80 bg-stone-50/40 dark:bg-stone-900/10">
                <div className="max-w-6xl mx-auto flex flex-col gap-10">
                  <div className="text-center">
                    <span className="text-[10px] font-mono tracking-widest text-[#A35638] font-black uppercase">Consignee Feedback</span>
                    <h3 className="font-sans font-black text-[#111E19] dark:text-stone-100 text-2xl md:text-3xl mt-1 leading-tight uppercase">Trusted By Regional Enterprises</h3>
                    <p className="text-xs text-stone-500 dark:text-stone-400 mt-2 max-w-xl mx-auto leading-normal">
                      Read transparent testimonials from logistics directors and procurement leads managing bilateral cargo streams.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white dark:bg-[#0c1411] border border-stone-200/60 dark:border-stone-800 rounded-3xl p-6 shadow-3xs flex flex-col justify-between hover:shadow-xs transition-shadow">
                      <div className="flex flex-col gap-4">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs font-mono font-bold text-stone-500 dark:text-stone-400 ml-1.5">(4.0)</span>
                        </div>
                        <p className="text-xs text-stone-600 dark:text-stone-300 italic leading-relaxed">
                          "Excellent parcel delivery and freight handoff services. The consignments are checked with supreme security at each major hub. High sorting efficiency and very prompt service when tracking with their client dashboard!"
                        </p>
                      </div>
                      <div className="mt-6 pt-4 border-t border-stone-100 dark:border-stone-800/80 flex items-center gap-3">
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-sans font-black text-xs text-[#111E19] dark:text-stone-100 uppercase truncate">Jaime Guaman</span>
                          <span className="text-[10px] text-stone-400 dark:text-stone-500 font-mono mt-0.5 truncate">Andes Import logistics &bull; Operations</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#0c1411] border border-stone-200/60 dark:border-stone-800 rounded-3xl p-6 shadow-3xs flex flex-col justify-between hover:shadow-xs transition-shadow">
                      <div className="flex flex-col gap-4">
                        ...
                        <p className="text-xs text-stone-600 dark:text-stone-300 italic leading-relaxed">
                          "Crest's overland raw materials route from the Montreal ports is highly secure. Our sorting compliance checks saw a 20% efficiency surge. Highly prompt, though trans-Canada rail crossings take their usual logistics window."
                        </p>
                      </div>
                      <div className="mt-6 pt-4 border-t border-stone-100 dark:border-stone-800/80 flex items-center gap-3">
                        <img 
                          src="https://images.unsplash.com/photo-1590086782957-93c060218022?auto=format&fit=crop&w=120&h=120&q=80" 
                          alt="John Wilson" 
                          className="w-10 h-10 rounded-full object-cover border border-stone-205 dark:border-stone-800"
                          referrerPolicy="no-referrer"
                        />
                        <div className="flex flex-col min-w-0">
                          <span className="font-sans font-black text-xs text-[#111E19] dark:text-stone-100 uppercase truncate">John Wilson</span>
                          <span className="text-[10px] text-stone-400 dark:text-stone-500 font-mono mt-0.5 truncate">Great Lakes Freight &bull; Supply Chain</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-[#0c1411] border border-stone-200/60 dark:border-stone-800 rounded-3xl p-6 shadow-3xs flex flex-col justify-between hover:shadow-xs transition-shadow">
                      <div className="flex flex-col gap-4">
                        <div className="flex items-center gap-1">
                          ...
                          <Star className="w-4 h-4 text-stone-300 dark:text-stone-700" />
                          <Star className="w-4 h-4 text-stone-300 dark:text-stone-700" />
                          <span className="text-xs font-mono font-bold text-stone-500 dark:text-stone-400 ml-1.5">(3.0)</span>
                        </div>
                        <p className="text-xs text-stone-600 dark:text-stone-300 italic leading-relaxed">
                          "Consistent overland distribution and smart warehousing. The physical handoffs at Saint John Port are extremely professional. I'd love more direct API integration routes for enterprise inventory syncing."
                        </p>
                      </div>
                      <div className="mt-6 pt-4 border-t border-stone-100 dark:border-stone-800/80 flex items-center gap-3">
                        <img 
                        ...
                        />
                        <div className="flex flex-col min-w-0">
                          <span className="font-sans font-black text-xs text-[#111E19] dark:text-stone-100 uppercase truncate">Carlos Jackson Hewitt</span>
                          <span className="text-[10px] text-stone-400 dark:text-stone-500 font-mono mt-0.5 truncate">Atlantic Maritime &bull; Procurement Dir</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col gap-8 lg:max-w-[55%]">
                    <div>
                      ...
                      <h3 className="font-sans font-black text-[#111E19] dark:text-stone-100 text-2xl md:text-3xl lg:text-4xl mt-1.5 leading-tight uppercase tracking-tight">
                        Our Commitment <br className="hidden lg:block"/>to Excellence
                      </h3>
                      <p className="text-xs text-stone-500 dark:text-stone-400 mt-3 leading-relaxed max-w-xl font-sans font-medium">
                        Crest operates under a rigorous corporate framework aimed at delivering high-capacity logistics solutions across Canada and worldwide. We continuous-audit each transit node to keep operations pristine and completely flawless.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-6">
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="flex flex-col gap-1">
                          <h4 className="font-sans font-extrabold text-sm text-[#111E19] dark:text-stone-100 uppercase tracking-wider">Expertise & Experience</h4>
                          <p className="text-xs text-stone-600 dark:text-stone-400 leading-relaxed font-sans">
                            Striving for perfection in every aspect of our business. With decades of multi-modal corridor coordination and robust freight handlers, our specialists maintain continuous oversight to guide your cargo safely past inland crossings and sea harbors.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="flex flex-col gap-1">
                          <h4 className="font-sans font-extrabold text-sm text-[#111E19] dark:text-stone-100 uppercase tracking-wider">Customer Centricity</h4>
                          <p className="text-xs text-stone-600 dark:text-stone-400 leading-relaxed font-sans">
                            Always putting our clients' needs first. Working together collaboratively to achieve shared business outcomes, backed by predictive waybill calculators, visual cargo route grids, and proactive custom dispatch scripts.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="flex flex-col gap-1">
                          <h4 className="font-sans font-extrabold text-sm text-[#111E19] dark:text-stone-100 uppercase tracking-wider">Sustainable Practices</h4>
                          <p className="text-xs text-stone-600 dark:text-stone-400 leading-relaxed font-sans">
                            Minimising environmental impact with green initiatives. We employ intelligent consolidation algorithms to optimize payload metrics on heavy transits, preventing unnecessary emissions while keeping distribution at state-of-the-art heights.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex-1 flex flex-col gap-6 lg:max-w-[55%]">
                    <div className="flex flex-col gap-2">
                      ...
                      <p className="text-sm text-yellow-100/90 leading-relaxed font-sans mt-2">
                        We deploy certified logistical personnel at the primary ocean container stacks to perform physical check-ins. Working hand-in-hand with regional port administrations, Crest verifies mechanical security locks, handles customs clearances, and synchronizes manifest weights prior to heavy overland transit.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col gap-5 mt-2">
                      ...
                      <div className="flex gap-4 items-start bg-[#182a20]/45 p-4 rounded-2xl border border-yellow-500/25">
                        ...
                        <div className="flex flex-col gap-0.5">
                          ...
                          <p className="text-xs text-yellow-100/80 leading-relaxed font-sans">
                            Each cargo container is tagged with high-security blockchain seal identifiers, physically verified under supervisor logging to eliminate any transport vulnerability.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start bg-[#182a20]/45 p-4 rounded-2xl border border-yellow-500/25">
                        ...
                        <div className="flex flex-col gap-0.5">
                          ...
                          <p className="text-xs text-yellow-100/80 leading-relaxed font-sans">
                            Dual-axis weighbridge records are synced with billing manifest bills of lading instantly, automatically triggering notifications if any payload discrepancy is flagged.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-4 items-start bg-[#182a20]/45 p-4 rounded-2xl border border-yellow-500/25">
                        ...
                        <div className="flex flex-col gap-0.5">
                          ...
                          <p className="text-xs text-yellow-100/80 leading-relaxed font-sans">
                            Close cooperation with local dock crews ensures immediate handoffs and rapid custom clearance, cutting maritime sorting delays by up to 34% annually.
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex-1 w-full lg:max-w-[45%] rounded-3xl overflow-hidden border-2 border-yellow-400/40 shadow-2xl relative flex bg-[#162720]">
                    ...
                    <div className="absolute bottom-4 left-4 bg-[#111E19]/90 border border-yellow-400/35 backdrop-blur-md p-3 rounded-2xl flex items-center gap-2.5 shadow-lg">
                      ...
                      <div className="flex flex-col select-none">
                        <span className="text-[7.5px] font-mono tracking-widest text-[#F7E4A1] uppercase font-bold leading-none">Status</span>
                        <span className="text-[10px] font-sans font-black text-white leading-none mt-1">DOCKSIDE INSPECTION IN-PROGRESS</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <p className="text-stone-600 text-sm font-sans leading-relaxed">
                    Crest Logistics is an international multi-modal shipping organization. Founded conceptually in 25 and expanding rapidly inside West-Central Africa, we establish secure inland heavy transit loops, deep container freighting, and cargo sorting pipelines powered by real-time metadata indexing features.
                  </p>

                  <div className="flex flex-col gap-2.5 mt-2 text-xs font-sans">
                    <div className="flex items-center gap-3">
                      <span className="w-5 h-5 rounded-full bg-[#F7E4A1] text-[#111E19] flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                      <span className="font-extrabold text-[#111E19]">Precision Dry Bulk Sorting</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="w-5 h-5 rounded-full bg-[#F7E4A1] text-[#111E19] flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                      <span className="font-extrabold text-[#111E19]">Cross-Border Ingress Customs Clearance Protocols</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="w-5 h-5 rounded-full bg-[#F7E4A1] text-[#111E19] flex items-center justify-center font-bold text-xs shrink-0">✓</span>
                      <span className="font-extrabold text-[#111E19]">Bilateral SMS/WhatsApp Consignee Alert Systems</span>
                    </div>
                  </div>
                </div>
            ...
            <section id="services" className="py-20 bg-white border-y border-stone-200/50 px-6 scroll-mt-14">
              <div className="max-w-7xl mx-auto flex flex-col gap-12">
                <div className="text-center max-w-2xl mx-auto">
                  <span className="text-xs uppercase font-mono tracking-widest text-[#A35638] font-black">Global Grids</span>
                  <h2 className="font-sans font-black text-[#111E19] text-3xl mt-2">Comprehensive Solutions For Enterprise</h2>
                  <p className="text-xs text-stone-500 mt-2">
                    No matter the scale of bulk or hazardous cargo, we operate custom pathways to secure delivery.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div className="bg-[#FAF5E9]/55 border border-stone-200/60 rounded-3xl p-6 flex flex-col justify-between gap-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col gap-4">
                      ...
                      <div>
                        <h3 className="font-sans font-black text-sm text-[#111E19] uppercase tracking-wide">Priority Air Freight</h3>
                        <p className="text-[11px] text-stone-500 font-sans mt-1 leading-normal">
                          Interstate express delivery mapping high-value electronics and refrigerated medical samples seamlessly.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-[#FAF5E9]/55 border border-stone-200/60 rounded-3xl p-6 flex flex-col justify-between gap-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col gap-4">
                      ...
                      <div>
                        <h3 className="font-sans font-black text-sm text-[#111E19] uppercase tracking-wide">Ocean Containers</h3>
                        <p className="text-[11px] text-stone-500 font-sans mt-1 leading-normal">
                          Global freight liners dealing with large raw materials, heavy-duty solar battery arrays, and dry bulk containers.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-[#FAF5E9]/55 border border-stone-200/60 rounded-3xl p-6 flex flex-col justify-between gap-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col gap-4">
                      ...
                      <div>
                        <h3 className="font-sans font-black text-sm text-[#111E19] uppercase tracking-wide">Overland Freight</h3>
                        <p className="text-[11px] text-stone-500 font-sans mt-1 leading-normal">
                          Robust heavy duty transport vehicles bridging Cameroon hubs directly with Central African Republic borders.
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="bg-[#FAF5E9]/55 border border-stone-200/60 rounded-3xl p-6 flex flex-col justify-between gap-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col gap-4">
                      ...
                      <div>
                        <h3 className="font-sans font-black text-sm text-[#111E19] uppercase tracking-wide">Climatic Storage</h3>
                        <p className="text-[11px] text-stone-500 font-sans mt-1 leading-normal">
                          Secure thermostatic warehouse zoning ensuring fragile lithium or paint products remain in safe indices.
                        </p>
                      </div>
                    </div>
                    <span className="text-[9px] font-mono tracking-widest text-[#A35638] font-bold">Crest Smart Warehouse</span>
                  </div>
            ...
            <section id="track" className="py-20 px-6 max-w-4xl mx-auto scroll-mt-14">
              <div className="bg-white border border-stone-200/60 rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-7">
                ...
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-sans font-black text-[#111E19] tracking-tight uppercase">Crest Tracking Terminal</h2>
                  <p className="text-xs text-stone-500 font-sans mt-1 max-w-md mx-auto">
                    Verify bilateral bills of lading, current milestone timelines, and packing directives live.
                  </p>
                </div>
                  ...
                  <div className="border border-stone-200 rounded-3xl p-5 md:p-6 flex flex-col gap-6 font-sans">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-4 border-b border-stone-100 gap-3">
                      <div>
                        ...
                        <h3 className="font-sans font-black text-lg text-[#A35638] tracking-tight">{publicFoundShipment.orderId}</h3>
                      </div>
                      <div className="flex flex-col sm:items-end">
                        <span className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">Current Milestone</span>
                        <span className={`px-3 py-1 bg-amber-50 rounded-full text-xs font-black uppercase inline-block text-center mt-1 border ${
                          publicFoundShipment.status === "MANIFEST_CREATED" ? "bg-stone-100 border-stone-200 text-stone-600" :
                          publicFoundShipment.status === "DRY_BULK_SORTED" ? "bg-amber-50 border-amber-250 text-amber-800" :
                          publicFoundShipment.status === "GATEWAY_CUSTOMS_HOLD" ? "text-rose-800 bg-rose-50 border-rose-200" :
                          publicFoundShipment.status === "DELIVERED" ? "text-emerald-800 bg-emerald-50 border-emerald-250" :
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-6 border-b border-stone-100 text-xs">
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 bg-stone-50/70 p-4 border border-stone-200/50 rounded-2xl">
                          <div>
                            <span className="text-[9px] uppercase tracking-wider text-stone-400 font-bold">Consignee</span>
                            <p className="font-bold text-[#111E19] mt-0.5">{publicFoundShipment.customerName}</p>
                          </div>
                          <div>
                            <span className="text-[9px] uppercase tracking-wider text-stone-400 font-bold">Weight</span>
                            <p className="font-mono text-[#111E19] mt-0.5">{publicFoundShipment.weight} kg</p>
                          </div>
                          <div className="col-span-2 pt-2 border-t border-stone-200/50">
                            <span className="text-[9px] uppercase tracking-wider text-stone-400 font-bold">Destination Terminal</span>
                            <p className="font-bold text-stone-600 mt-0.5 leading-relaxed">{publicFoundShipment.destination}</p>
                          </div>
                        </div>
                      </div>

                      <div className="flex flex-col gap-3">
                        <h4 className="font-extrabold text-[#111E19]">Cargotech Packing Checklists</h4>
                        <div className="bg-[#FAF5E9]/50 border border-stone-250 p-4 rounded-2xl flex flex-col gap-2">
                          <p className="text-[9px] font-mono uppercase text-amber-700 font-bold">Cargo manifest lines ({publicFoundShipment.items.length})</p>
                          <div className="flex flex-col gap-1 max-h-24 overflow-y-auto">
                            {publicFoundShipment.items.map((it: any, i: number) => (
                              <div key={i} className="flex justify-between items-center text-[11px] leading-relaxed">
                                <span className="text-stone-700">{it.name || "Cargo Pack Unit"}</span>
                                <span className="font-mono bg-white border border-stone-200 px-1.5 py-0.2 rounded font-bold text-stone-800">x{it.qty}</span>
                              </div>
                            ))}
                          </div>
                          {publicFoundShipment.fragile && (
                            <span className="bg-rose-50 text-rose-800 border-rose-150 inline-block text-[9px] uppercase font-bold tracking-wider rounded px-2.1 py-0.5 mt-1 border">
                              ⚠️ FRAGILE: High-Security Insulation Checklist Active
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="relative pl-6 flex flex-col gap-6 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-stone-200">
                        {publicFoundShipment.history && publicFoundShipment.history.map((hist: any, i: number) => {
                          const isActive = i === publicFoundShipment.history.length - 1;
                          return (
                            <div key={i} className="relative flex flex-col gap-1 text-xs">
                              ...
                              <div className="flex justify-between items-baseline flex-col sm:flex-row gap-0.5">
                                <span className={`font-mono text-[10px] font-black uppercase ${isActive ? "text-[#A35638]" : "text-stone-500"}`}>
                                  {hist.status.replace(/_/g, " ")} — {hist.location}
                                </span>
                                <span className="text-[9px] text-stone-400 font-mono">{hist.date}</span>
                              </div>
                              <p className="text-[11px] text-stone-600 leading-normal">{hist.description}</p>
                            </div>
                          );
                        })}
                      </div>
                    ...
                    <div className="bg-[#FAF4E8] rounded-2xl border border-stone-250 p-4 flex flex-col gap-2.5">
                      ...
                      <p className="text-[11px] font-sans text-stone-700 leading-relaxed italic">
                        &ldquo;Crest AI Agent Dispatch Analysis: Verified waypoint metrics for consignment {publicFoundShipment.orderId}. Hand-off optimization indicates carrier line: {publicFoundShipment.insights?.suggestedCarrier || "DHL Freight"}. Estimated transit window parameter is {publicFoundShipment.insights?.predictedTransitDays || "3 days"}. Security risk class evaluated at {publicFoundShipment.insights?.riskAssessment || "LOW"}. Consignee or customs seals processing is in order. No customer actions required from your side.&rdquo;
                      </p>
                    </div>
          ...
          {activeTab === "contact" && (
            <section id="contact" className="py-20 md:py-24 px-6 max-w-4xl mx-auto scroll-mt-14">
              <div className="bg-white border border-stone-200/60 rounded-3xl p-6 md:p-8 shadow-sm flex flex-col gap-6">
                ...
                <div className="text-center">
                  <span className="text-xs uppercase font-mono tracking-widest text-[#A35638] font-black block">Get a Free Estimate</span>
                  ...
                  <p className="text-xs text-stone-500 font-sans mt-1">
                    Receive verified transit schedules, packing containers cost grids, and optimized carrier channels.
                  </p>
                </div>
                ...
                {contactSubmitted ? (
                  <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl p-6 text-center text-xs font-sans font-medium animate-fadeIn">
                    ...
                    Thank you! Your quote request has been queued. Our dispatch routing desk will reach you via SMS shortly!
                  </div>
                ) : (
                  <form onSubmit={handleContactSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-sans">
                    <div>
                      <label className="block text-stone-500 font-bold mb-1 uppercase tracking-wider text-[10px]">Your Name</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-stone-500 font-bold mb-1 uppercase tracking-wider text-[10px]">Your Email</label>
                      <input
                        type="email"
                        className="w-full bg-stone-50/50 border border-stone-200 px-3.5 py-2.5 rounded-xl focus:outline-[#A35638]"
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-stone-500 font-bold mb-1 uppercase tracking-wider text-[10px]">Phone Number (For dispatch alerts)</label>
                      <input
                        type="text"
                        className="w-full bg-stone-50/50 border border-stone-200 px-3.5 py-2.5 rounded-xl focus:outline-[#A35638]"
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-stone-500 font-bold mb-1 uppercase tracking-wider text-[10px]">Crate Manifest Cargo Properties & Destinations</label>
                      <textarea
                        rows={4}
      ...
      <footer className="w-full bg-[#111E19] text-white pt-16 pb-8 px-6 border-t border-stone-900 mt-auto font-sans">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 pb-10 border-b border-stone-800">
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-[11px] text-stone-400 leading-normal max-w-xs">
              Autonomous logistics algorithms, bilateral communication portals, and dry-bulk cargo safety seals.
            </p>
          </div>

          <div>
            <h4 className="font-black uppercase tracking-wider text-xs mb-4 text-[#F7E4A1]">Corporate Segments</h4>
            <div className="flex flex-col gap-2.5 text-xs text-stone-400">
              <span className="cursor-pointer hover:underline" onClick={() => navigateToSegment("home")}>Global Home</span>
              <span className="cursor-pointer hover:underline" onClick={() => navigateToSegment("about")}>Our Mission Chronology</span>
              <span className="cursor-pointer hover:underline" onClick={() => navigateToSegment("services")}>Freight Solutions Grid</span>
              <span className="cursor-pointer hover:underline" onClick={() => { setActiveTab("track"); setIsAdminMode(false); }}>Tracking Console ID Search</span>
            </div>
          </div>

          {!forcePublic && !forceAdminLogin && !forceAdminDashboard && (
            <div>
              <h4 className="font-black uppercase tracking-wider text-xs mb-4 text-[#F7E4A1]">Enterprise Access</h4>
              <div className="flex flex-col gap-2.5 text-xs text-stone-400">
                <button 
                ...
                <span className="text-[11px] text-stone-500">Demo Code PIN: admin2026</span>
              </div>
            </div>
          )}

          <div>
            <h4 className="font-black uppercase tracking-wider text-xs mb-4 text-[#F7E4A1]">Terminal Check-in</h4>
            <p className="text-xs text-stone-400 leading-relaxed mb-1">
              Main Regional Ingress Portal Address:
            </p>
            <p className="text-xs text-emerald-400 font-mono">
              Tower Suite B, Rue de Douala Hub, Yaoundé, Cameroon
            </p>
          </div>
        ...
        <div className="max-w-7xl mx-auto pt-6 flex flex-col sm:flex-row justify-between items-center text-[10px] text-stone-500 font-mono gap-4">
          <span>© 2026 Crest Logistics International Corp. All legal corporate rights reserved.</span>
          ...
          <div className="w-80 h-96 bg-white border border-stone-200/80 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-fadeIn">
            ...
            <div className="bg-[#111E19] text-white p-4 flex justify-between items-center shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-black text-xs uppercase tracking-wide">Crest Cog Agent</h4>
                  <p className="text-[9px] text-emerald-400 font-mono leading-none mt-0.5">ONLINE SUPPORT</p>
                </div>
              </div>
              <button onClick={() => setChatOpen(false)} className="text-stone-300 hover:text-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 text-xs bg-[#FAF5E9]/35">
              {chatHistory.map((item, index) => (
                ...
                >
                  <p className="leading-relaxed text-[11px] whitespace-pre-wrap">{item.text}</p>
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4 transition-all">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-stone-200/80 flex flex-col gap-5 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="text-xs text-stone-600 font-sans leading-relaxed">
              Are you absolutely sure you want to permanently remove this shipment from server records?
              <div className="mt-2.5 bg-stone-50 border border-stone-100/60 rounded-xl p-3 flex flex-col gap-1.5 font-medium text-stone-700">
                ...
                <p className="flex justify-between">
                  <span className="text-stone-400 text-[10px]">Origin Hub:</span>
                  <span className="truncate max-w-[150px]">{shipmentToDelete.origin || "California, USA"}</span>
                </p>
                <p className="flex justify-between">
                  <span className="text-stone-400 text-[10px]">Destination:</span>
                  <span className="truncate max-w-[150px]">{shipmentToDelete.destination}</span>
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans text-[#111E19]">
              ...
              <div className="col-span-2">
                ...
                <input
                  ...
                  onChange={(e) => setEditingShipment({ ...editingShipment, customerName: e.target.value })}
                />
              </div>
              ...
              <div>
                ...
                <div className="flex rounded-xl overflow-hidden border border-stone-300">
                  <input
                    ...
                    value={editingShipment.packageWeight !== undefined ? editingShipment.packageWeight : editingShipment.weight || ""}
                  ...
                  <select
                    ...
                    onChange={(e: any) => setEditingShipment({ ...editingShipment, weightUnit: e.target.value })}
                  >
                    <option value="kg">kg</option>
                    <option value="lbs">lbs</option>
                  </select>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditingShipment({ ...editingShipment, paymentMethod: e.target.value })}
                >
                  <option value="Apple Pay">Apple Pay</option>
                  <option value="Credit Card">Credit Card</option>
                  <option value="Cash App">Cash App</option>
              ...
              <div className="flex flex-col justify-center">
                ...
                <label className="flex items-center gap-2 cursor-pointer font-bold select-none text-[#111E19]">
                  <input
                    ...
                    onChange={(e) => setEditingShipment({ ...editingShipment, fragile: e.target.checked })}
                  />
              ...
              <div className="grid grid-cols-2 gap-2 border border-dashed border-stone-250 rounded-xl p-2.5 bg-stone-50 col-span-2">
                <div className="col-span-2 flex items-center gap-1.5">
                  ...
                  <span className="block text-[8px] font-sans font-extrabold uppercase text-stone-400">🚗 Operational Overland Progress</span>
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={editingShipment.hoursDriven !== undefined ? editingShipment.hoursDriven : 0}
                    onChange={(e) => setEditingShipment({ ...editingShipment, hoursDriven: parseInt(e.target.value) || 0 })}
                  />
                </div>
                <div>
                  <label className="block text-[9px] font-sans font-bold uppercase text-stone-500 mb-0.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. 400"
                    className="w-full text-xs font-mono rounded-lg border border-stone-300 px-2 py-1.5 bg-white text-stone-900 focus:outline-[#A35638]"
                    value={editingShipment.distanceCovered !== undefined ? editingShipment.distanceCovered : 0}
                    onChange={(e) => setEditingShipment({ ...editingShipment, distanceCovered: parseFloat(e.target.value) || 0 })}
                  />
                </div>
              </div>
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    if (!response.ok) {
                      throw new Error("Failed to edit shipment details on server.");
                    }
                    ...
                    if (lastCreatedDomesticWaybill && lastCreatedDomesticWaybill.orderId === updated.orderId) {
